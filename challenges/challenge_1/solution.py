...
def fedavg_aggregate(
    ...
    global_coef      = np.zeros_like(client_params[0]["coef"])
    global_intercept = np.zeros_like(client_params[0]["intercept"])

    for p in client_params:
        weight = p["n_samples"] / total_samples
        global_coef      += weight * p["coef"]
        global_intercept += weight * p["intercept"]
